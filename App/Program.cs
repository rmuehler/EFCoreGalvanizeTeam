...
namespace EFCoreGalvanizeTeam
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var student in db.Students.Select(student =>
                new {fName = student.FirstName, lName = student.LastName}))
            {
            ...
            var student3 = db.Students.Include(student => student.Grades)
                .OrderByDescending(student => student.Grades.Average(grade => (float) grade.Value))
            ...
            //8
            Console.Out.WriteLine("\nFind the average grades of Sophomores");
                ...
                .Where(student => student.Classification == Student.Class.Sophomore).Select(student =>
                    new
                        {FirstName = student.FirstName,
                            ...
                            Average = student.Grades.Average(grade => (float) grade.Value)})
                .ToList();
            sophomoreAverages.ForEach(student =>
                Console.WriteLine($"{student.FirstName} {student.LastName} has an average of {student.Average}"));
        }
    }
}